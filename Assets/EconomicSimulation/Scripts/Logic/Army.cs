...
namespace Nashet.EconomicSimulation
{
    public class Army : Name
    {
		...
		static Province lastSendingTarget;
        ...
        {
            if (e.oldOwner == Province.Country && Path != null && Path.Any(x => x.Province == sender))//changed owner, probably, on our way
            {
                Path = null;
                Game.provincesToRedrawArmies.Add(Province);
                //Province.RedrawLocalArmies(); 
        ...
        public void AddToPath(Province destinationProvince)
        {
            ...
            {
                List<Node> extrapath = null;
                PathFinder.instance.FindShortestPathOfNodes(Path.Last().autoGeneratedID, destinationProvince.Node.autoGeneratedID,
                    Execution.Synchronous, x => extrapath = x);
                Path.AddRange(extrapath);
                Game.provincesToRedrawArmies.Add(Province);
            }
        }
		...
		public void SetPathTo(Province destinationProvince, Predicate<IProvince> predicate = null)
        {
            ...
            {
				void callback(List<Node> x)
                {
                    Path = x;
                    if (Path == null && owner == Game.Player && !Game.Player.isAI())
                    {
                        if (destinationProvince != lastSendingTarget)
                        {
                            MessageSystem.Instance.NewMessage("Do you know de way?", "Commander, we can't find de way. Try to attack enemy first or hold ALT key. Or may be that route is just impassable", "Fine", false, destinationProvince.provinceMesh.Position);							
						}
                    }
					lastSendingTarget = destinationProvince;
				};
				PathFinder.instance.FindShortestPathOfNodes(Province.Node.autoGeneratedID,
                    destinationProvince.Node.autoGeneratedID, Execution.Synchronous, callback, predicate);
            }
            Game.provincesToRedrawArmies.Add(Province);
            //Province.RedrawLocalArmies();
        }
