...
namespace Nashet.EconomicSimulation
{
    ...
    public class Game : ThreadedJob
    {
		...
		private static void AddRivers()
		{
			for (int i = 0; i < 17; i++)
			{
				var riverStart = World.AllProvinces.Where(x => x.IsCoastal && !x.AllNeighbors().Any(y => y.isRiverNeighbor(x))).Random();
				//x.Terrain == Province.TerrainTypes.Mountains &&
				var riverStart2 = riverStart.AllNeighbors().Where(x => x.IsCoastal).Random();
                if (riverStart == null || riverStart2 == null) 
                    continue;
