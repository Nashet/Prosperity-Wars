...
namespace Nashet.Map.Examples
{
	...
	public class MapGenerator : MonoBehaviour
	{
		...
		private int amountOFCountries = 2;
		...
		public Rect GenerateWorld()
		{
			...
			{
				CreateProvince(grid, meshes, colorId);
			}
		...
		private void CreateProvince(VoxelGrid grid, Dictionary<int, Dictionary<int, MeshStructure>> meshes, Color colorId)
		{
			//country.color.getAlmostSameColor()
		...
		private static void SetPatchFinding(Dictionary<int, Dictionary<int, MeshStructure>> meshes)
		{
			...
			foreach (var province in Province.AllProvinces)
			{
				foreach (var neighborId in meshes[province.Key])
				{
					...
					if (PathFinder.instance.graphData.paths.Exists(x => x.IDOfA == province.Value.Node.autoGeneratedID && x.IDOfB == neighbor.Node.autoGeneratedID))
						continue;
					PathFinder.instance.graphData.paths.Add(
						new Path(neighbor.Node.autoGeneratedID, province.Value.Node.autoGeneratedID));
				}
			}
		}
